...
import { useSellerOrderQuery, useUpdateOrderStatusMutation } from '@/hooks/api';
import { colors, radius, spacing, typography } from '@/theme';
import type { OrderStatus } from '@/types/api';
import { getErrorMessage } from '@/utils/apiErrors';
import { formatBRL, formatOrderStatus } from '@/utils/format';

type Route = RouteProp<{ SellerOrderDetails: { orderId: string } }, 'SellerOrderDetails'>;
...
export const SellerOrderDetailsScreen = () => {
  const route = useRoute<Route>();
  const navigation = useNavigation();
  const { orderId } = route.params;

  const orderQuery = useSellerOrderQuery(orderId);
  ...
  const onAdvance = () => {
    ...
    Alert.alert(
      'Atualizar status?',
      `Mover para "${formatOrderStatus(next)}"?`,
      [
    ...
    <AppScreen>
      ...
      {next ? (
        <View style={styles.footer}>
          <Button
            label={`Mover para "${formatOrderStatus(next)}"`}
            fullWidth
        ...
        <View style={styles.footer}>
          <View style={styles.doneRow}>
            ...
            <Text style={styles.doneText}>Sem mais transições disponíveis.</Text>
          </View>
        </View>
      )}
    </AppScreen>
...
const styles = StyleSheet.create({
  ...
  amountSep: { height: 1, backgroundColor: colors.border, marginVertical: spacing.xs },
